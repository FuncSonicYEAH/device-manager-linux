...
Qt 的符號版本向下相容，因此想涵蓋盡量多的系統，就在所要支援的最老發行版上建置：
在 Debian trixie（Qt 6.8.2）上建置的二進位檔也能在裝有 Qt 6.11 的 Fedora/Arch 上執行，
反之則會報 `version Qt_6.11 not found`（已在 distrobox 容器中實測）。
