...
Qt 的符号版本向下兼容，因此想覆盖尽量多的系统，就在所要支持的最老发行版上构建：
在 Debian trixie（Qt 6.8.2）上构建的二进制也能在装有 Qt 6.11 的 Fedora/Arch 上运行，
反之则会报 `version Qt_6.11 not found`（已在 distrobox 容器中实测）。
